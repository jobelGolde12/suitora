    ...
    fetch("/api/stylist", { credentials: "include" })
      ...
      .catch(() => {
        if (isMounted.current) {
          addToast("Failed to load your stylist history", "error");
        }
      })
      .finally(() => {
          ...
          <div className="space-y-6">
            <div className="rounded-2xl bg-surface border border-border/60 p-4">
              <p className="text-sm text-muted font-light leading-relaxed">
                Hi — I&apos;m your AI fashion stylist. Ask me anything about
                fit, color, or putting outfits together, and I&apos;ll tailor
                my advice to your profile and past analyses.
              </p>
            </div>
            <div className="flex flex-wrap gap-2">
              {SUGGESTED_PROMPTS.map((prompt) => (
                <button
                  ...
                  disabled={isSending}
                  className="rounded-full border border-border bg-card px-4 py-2 text-xs text-muted transition-colors duration-200 hover:border-accent/40 hover:text-foreground cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
                >
                  {prompt}
                </button>
              ))}
                  ...
                  <div className="flex max-w-[95%] flex-wrap gap-2">
                    {msg.actions?.map((action) => (
                      <Link
                        ...
                        href={action.href}
                        className="inline-flex items-center gap-1 rounded-full border border-accent/40 bg-accent/10 px-3 py-1.5 text-xs font-medium text-accent transition-colors hover:bg-accent/20 focus:outline-none focus-visible:ring-2 focus-visible:ring-ring"
                      >
                        {action.label}
                    ...
                    {msg.followUps?.map((chip) => (
                      <button
                        ...
                        disabled={isSending}
                        className="rounded-full border border-border bg-card px-3 py-1.5 text-xs text-muted transition-colors hover:border-accent/40 hover:text-foreground cursor-pointer focus:outline-none focus-visible:ring-2 focus-visible:ring-ring disabled:opacity-50"
                      >
                        {chip}
                      </button>
                    ))}
