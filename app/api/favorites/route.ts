...
export const GET = withApiRoute("/api/favorites", async () => {
...
export const POST = withApiRoute("/api/favorites", async (req: Request) => {
  const user = await requireUser();
  if (!user) {
    return apiError("Unauthorized", 401);
  }
...
export const PATCH = withApiRoute("/api/favorites", async (req: Request) => {
  const user = await requireUser();
  if (!user) {
    return apiError("Unauthorized", 401);
  }
...
export const DELETE = withApiRoute("/api/favorites", async (req: Request) => {
  const user = await requireUser();
  if (!user) {
    return apiError("Unauthorized", 401);
  }
