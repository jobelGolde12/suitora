...
async function fetchWithRetry(url: string, init: RequestInit): Promise<Response> {
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    ...
    if (attempt < 2) {
      await new Promise((resolve) => setTimeout(resolve, 1_000 * 2 ** attempt + Math.random() * 500));
    }
  }
...
function buildSystemPrompt(context: StylistContext): string {
  const lines: string[] = [
  ...
  if (context.styleTags?.length) profile.push(`Style preferences: ${context.styleTags.join(", ")}`);
  ...
  if (context.totalAnalyses > 0) {
    ...
    if (context.bestScore != null) history.push(`Best score: ${Math.round(context.bestScore)}`);
  }
...
async function callOpenAI(messages: StylistMessageInput[], context: StylistContext): Promise<string> {
...
function mockStylistReply(messages: StylistMessageInput[], context: StylistContext): string {
  ...
  if (text.includes("color") || text.includes("colour") || text.includes("palette")) {
    if (context.skinTone) {
  ...
  if (text.includes("what") && text.includes("wear") || text.includes("occasion") || text.includes("event")) {
  ...
  const recent = context.recentScores.length ? ` your last ${context.recentScores.length} scores averaged ${Math.round(context.averageScore)}` : "";
